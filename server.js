...
// Rooms and queues
const rooms = {}; // roomName -> { users: [], queue: [], currentSong: null }
...
io.on("connection", (socket) => {
  ...
  socket.on("joinRoom", ({ room, nickname }) => {
    socket.join(room);
    if (!rooms[room]) rooms[room] = { users: [], queue: [], currentSong: null };
    rooms[room].users.push({ id: socket.id, nickname });
